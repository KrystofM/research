...
class Vertex(object):
    ...
    def _add_incidence(self, edge: "Edge"):
        ...
        self._incidence_labels.add(other.label)

    def del_edge(self, edge: "Edge"):
        self._incidence.pop(edge.other_end(self))
        self._incidence_labels.remove(edge.other_end(self).label)
    ...
    @property
    def neighbours_labels(self):
        return self._incidence_labels
    @property
    def degree(self) -> int:
        """
...
class Graph(object):
    ...
    def __repr__(self):
        """
        A programmer-friendly re--presentation of the Graph.
    ...
    def del_edge(self, edge: Edge):
        self._e.remove(edge)
        edge.head.del_edge(edge)
        edge.tail.del_edge(edge)
    ...
    @property
    def vertices_reference(self) -> List["Vertex"]:
        return self._v
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        G = Graph(False, len(self.vertices) + len(other.vertices))
        new_labels = {}
        label = 0
        ...
        for e in (self.edges + other.edges):
            G.add_edge(Edge(G.vertices[new_labels[e.tail]], G.vertices[new_labels[e.head]]))
    ...
    def is_tree(self) -> bool:
        return len(self.vertices) == len(self.edges) + 1 and self.is_connected()

    def is_connected(self) -> bool:
        if(len(self.vertices) == 0):
            return True
        reached = set({self.vertices[0]})
        frontier = set({self.vertices[0]})
    ...
    def connected_components(self):
        ...
        left = set(self.vertices) # Vertices not part of any components
        components = []
        while len(left) > 0:
            initial_vertex = left.pop()
            reached = set({initial_vertex})
            frontier = set({initial_vertex})
            while len(frontier) > 0:
                current = frontier.pop()
                for n in current.neighbors:
                    if n not in reached:
            ...
            component = Graph(False)
            vertex_map = {}
            for v in reached:
                vn = Vertex(component)
                component.add_vertex(vn)
                vertex_map[v] = vn
            for e in self.edges:
                if e.head in reached:
                    component.add_edge(Edge(vertex_map[e.head], vertex_map[e.tail]))
